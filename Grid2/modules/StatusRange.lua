...
local Ranges= {
	...
	["30"] = function(unit) return IsItemInRange((UnitIsFriend(unit,"player") and 1180 or 835),unit)==1 end,	--	1180 = Scroll of Stamina, 835 = Large Rope Net
	["38"] = UnitInRange,
	["40"] = function(unit) return IsItemInRange((UnitIsFriend(unit,"player") and 34471 or 28767),unit)==1 end,	--	34471 = Vial of the Sunwell, 28767 = The Decapitator
	["99"] = UnitIsVisible,
}
...
function Range:Grid_GroupTypeChanged(_, groupType)
	...
	if (tostring(self.dbx.range) or "40") == "40" then
		self:UpdateDB()
	end
end
-- }}
...
function Range:UpdateDB()
	UnitRangeCheck = Ranges[tostring(self.dbx.range) or "40"] or Ranges["40"] 
...
Grid2:DbSetStatusDefaultValue( "range", {type = "range", range= 40, default = 0.25, elapsed = 0.5})